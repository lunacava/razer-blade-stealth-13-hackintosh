...
### ★ 追記 (00:36): 今夜の全 4 サイクル — 落ちたのは唯一の Idle Sleep

末尾の秒数は `pmset -g log` のその状態の継続時間（= 睡眠時間）。

| 入眠 | 種別 | 睡眠 | Wi-Fi |
|---|---|---|---|
| 23:30:23 | Software Sleep | 46s | ✅ 生存 |
| 23:55:42 | Clamshell Sleep | 4s | ✅ 生存 |
| **00:09:07** | **Idle Sleep** | **115s** | **❌ 落ちた** |
| 00:34:20 | Software Sleep | 17s | ✅ 生存 |

00:34:37 の復帰後の実測: `en0 status: active`、20 ping で 0.0% loss / avg 18.0ms
（00:17 時点は 0.0% loss だが avg 84ms / max 515ms とスパイキーだった）。
`airportd` ログに 00:15 以降 link down / disassoc / deauth は**ゼロ**、
00:34:37.699 の `airportdProcessDLILEvent: <en0> Setting power ON` のみ。

**3 勝 1 敗。これは反例ではなく 4 番目のサンプルである。** 発見 25 は最初から
間欠故障として記録してあるので、成功 1 回はそれを否定しない（有線不要と誤判断した
時と同じ論法になる）。

ただし敗けた 1 回には意味のある差がある:

- 落ちたのは**唯一の Idle Sleep**（放置 10 分での自動入眠）で、睡眠時間も最長
- 生き残った 3 回はすべて手動 sleep / クラムシェルで **4〜46 秒**

→ **仮説: 落ちるのは「放置して長く寝た後」。** 短時間 sleep が通ることは保証にならない。
遠隔操作の用途は「長時間放置した後に叩く」ことなので、これは最悪ケースと一致する。

**決着に必要な未実施テスト**: 30 分以上触らず Idle Sleep させ、外から叩いて生死を見る。
**3 回連続クリーンで初めて**有線を外す判断材料になる。1 回では足りない。
