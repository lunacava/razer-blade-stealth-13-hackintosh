...
CPUFriend を入れるべきか判断するために、まず現状を測った。**結論として、
CPUFriend を無条件に入れるのは推奨できない**（理由は後述の「移植元がない」問題）。
...
有力候補が 2 つあり、まだ切り分けられていない。

1. **周波数ベクタ（EPP）のミスマッチ。** XCPM は上限 46 を許可していても、
   utilization に応じた要求値は周波数ベクタが決める。28W 4 コア向けの
   MacBookPro15,2 プロファイルが、単スレッド負荷に 46 を要求しないだけ、という説。
   → これなら CPUFriend で直る。
2. **マルチコア turbo 比率が適用されている。** i7-8565U の turbo ratio は
   1C=46 に対し 3C/4C=41 付近。他コアが深い C-state に入りきらず、
   パッケージが「3 コア以上アクティブ」と判断していると 41 が上限になる。
   → これは C-state 側の問題で、CPUFriend では直らない。

`0.4 秒バーストでも 4.08` という平坦さは、どちらかと言えば 2 を示唆する。
切り分けには `powermetrics` のコア別周波数と C-state residency が必要で、
**これには root が要る**。
...
無条件には入れない。まず上の「41 の理由」を切り分ける。切り分けには root が
必要なので、`/tmp/pmcap.sh` を用意した（アイドル / 単スレッド / 全 8 スレッドの
3 状態で `powermetrics --samplers cpu_power` を採取する）。
